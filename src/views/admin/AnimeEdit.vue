...
<template>
  <div>
    <div class="mb-8">
      <h2 class="text-2xl font-bold">Anime Düzenle</h2>
      <p class="text-sm text-muted-foreground">{{ anime?.title }} animesini düzenleyin.</p>
    </div>
...
const fetchAnime = async () => {
  try {
    ...
    // const response = await fetch(`/api/anime/${route.params.id}`)
    ...
    anime.value = {
      ...
      description: 'Tanjiro Kamado, ailesi bir şeytan tarafından katledildikten ve kız kardeşi Nezuko bir şeytana dönüştürüldükten sonra, kız kardeşini iyileştirmenin bir yolunu bulmaya ve ailesini öldüren şeytandan intikam almaya yemin eder.',
      year: 2019,
      season: 'spring',
      status: 'ongoing',
      type: 'tv',
      ...
      tags: ['demons', 'supernatural', 'martial arts'],
      ageRating: 'PG-13',
      contentWarnings: ['violence', 'gore']
    }

    form.value = { ...anime.value }
  } catch (error) {
